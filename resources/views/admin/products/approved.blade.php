...
<div class="container py-2 py-md-3">
    ...
    <h1 class="h2 fw-semibold mb-4">Approved products</h1>

    @if($approvedProducts->isEmpty())
        <div class="card">
            <div class="card-body text-center py-5">
                <i class="bi bi-box-seam text-muted" style="font-size: 3rem;"></i>
                <p class="mt-3 mb-0 text-muted">No approved products yet.</p>
            </div>
        </div>
    @else
        <div class="card">
                        ...
                        <tbody>
                            @foreach($approvedProducts as $product)
                                <tr>
                                    ...
                                    <td class="text-end">
                                        <a href="{{ route('products.show', $product) }}" class="btn btn-sm btn-outline-primary" target="_blank">View</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
        ...
        <div class="mt-3">{{ $approvedProducts->links() }}</div>
    @endif
</div>
@endsection
