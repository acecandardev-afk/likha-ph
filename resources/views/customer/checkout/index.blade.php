            ...
            <div class="row g-4">
                <div class="col-12 col-lg-8">
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            @if(auth()->user()->shipping_barangay || auth()->user()->shipping_address || auth()->user()->shipping_phone)
                            ...
                            <div class="mb-3">
                                ...
                                <x-guihulngan.barangay-select name="shipping_barangay" id="shipping_barangay" :value="old('shipping_barangay', auth()->user()->shipping_barangay)" :required="true" class="form-select @error('shipping_barangay') is-invalid @enderror" empty-label="Select barangay" />
                                <small class="text-muted">{{ config('guihulngan.city_name') }}, {{ config('guihulngan.province') }}</small>
                                @error('shipping_barangay')
                                    <span class="invalid-feedback d-block">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="mb-3">
                                <label for="shipping_address" class="form-label">Street, house no., landmarks <span class="text-muted fw-normal">(optional)</span></label>
                                <textarea name="shipping_address" id="shipping_address" rows="3" class="form-control @error('shipping_address') is-invalid @enderror" placeholder="Helps couriers find your location">{{ old('shipping_address', auth()->user()->shipping_address) }}</textarea>
                                @error('shipping_address')
                                    <span class="invalid-feedback">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="mb-0">
                                <label for="shipping_phone" class="form-label">Contact number for delivery <span class="text-danger">*</span></label>
                                <input type="text" name="shipping_phone" id="shipping_phone" class="form-control @error('shipping_phone') is-invalid @enderror" value="{{ old('shipping_phone') }}" required placeholder="+63 912 345 6789">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (useSavedBtn) {
        const savedBarangay = @json(auth()->user()->shipping_barangay ?? '');
        const savedAddress = @json(auth()->user()->shipping_address ?? '');
        const savedPhone = @json(auth()->user()->shipping_phone ?? '');
        useSavedBtn.addEventListener('click', function() {
            const brgy = document.getElementById('shipping_barangay');
            if (brgy) brgy.value = savedBarangay;
            document.getElementById('shipping_address').value = savedAddress;
            document.getElementById('shipping_phone').value = savedPhone;
        });
    }
});
</script>
